...
const Themes = () => {
  const themes = [
    {
      ...
      icon: Activity,
      summary:
        "Forecast battery degradation and Remaining Useful Life (RUL) using predictive algorithms.",
      problem:
        "Design a cloud-based system that collects real-time data on EV battery parameters and applies predictive algorithms to forecast battery degradation and remaining useful life (RUL).",
      objective:
        "Alert users and fleet operators about potential failures before they occur, ensuring safety and reduced downtime.",
      approach: [
    ...
    },
    {
      ...
      icon: Thermometer,
      summary:
        "Detect and analyze abnormal thermal patterns to predict thermal runaway.",
      problem:
        "Develop a monitoring solution that detects and analyzes abnormal thermal patterns in EV batteries using IoT sensors to identify hotspots and predict thermal runaway.",
      objective:
        "Predict thermal runaway and prevent catastrophic failures through real-time monitoring and alerting.",
      approach: [
    ...
    },
    {
      ...
      icon: BarChart3,
      summary:
        "Centralized dashboard for fleet operators to aggregate and analyze battery performance.",
      problem:
        "Create a centralized, cloud-based dashboard for fleet operators that aggregates battery performance data from multiple EVs, providing insights into usage and health.",
      objective:
  ...
  return (
    <div className="bg-transparent py-24 relative overflow-hidden" id="themes">
      <div className="mx-auto max-w-7xl px-6 lg:px-8 relative z-10">
        <div className="text-center mb-20">
          <Badge
            variant="outline"
            className="mb-4 border-brand-accent/30 text-brand-accent bg-brand-accent/5 tracking-widest uppercase"
          >
            Hackathon Tracks
          </Badge>
          ...
          <p className="text-lg text-brand-muted max-w-2xl mx-auto">
            We have defined three critical problem statements. Select the one
            that challenges you to push the boundaries of EV technology.
          </p>
        </div>
          ...
          {themes.map((theme, index) => (
            ...
            >
              <div className="h-full bg-brand-secondary/10 border border-brand-secondary hover:border-brand-accent/50 transition-all duration-300 rounded-2xl p-8 flex flex-col items-center text-center group relative overflow-hidden">
                <div className="absolute inset-0 bg-linear-to-b from-brand-accent/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"></div>
                ...
                <h3 className="text-xl font-bold text-white font-display uppercase mb-4 leading-tight">
                  {theme.title}
                </h3>
                    ...
                    <DialogContent className="bg-brand-secondary border-brand-secondary text-white sm:max-w-lg max-h-[90vh] overflow-y-auto z-100">
                      <DialogHeader className="text-left">
                        <DialogTitle className="text-2xl font-display uppercase flex items-start gap-3">
                          <theme.icon className="w-6 h-6 text-brand-accent shrink-0 mt-1" />
                          {theme.title}
                        </DialogTitle>
                      ...
                      <div className="space-y-6 pt-4 text-left">
                        ...
                        <div className="bg-brand-primary/50 p-4 rounded-lg border border-brand-secondary">
                          <h4 className="text-xs font-bold text-brand-muted uppercase mb-3">
